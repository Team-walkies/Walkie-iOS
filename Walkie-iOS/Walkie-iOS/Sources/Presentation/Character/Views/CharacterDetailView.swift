...
struct CharacterDetailView: View {
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ScrollView {
                VStack(spacing: 0) {
                    Image(viewModel.detailState.characterImage)
                        .resizable()
                        .frame(width: 180, height: 180)
                        .padding(.bottom, 12)
                    Text(viewModel.detailState.characterName)
                        .font(.H3)
                        .foregroundStyle(WalkieCommonAsset.gray700.swiftUIColor)
                        .padding(.bottom, 8)
                    Text(viewModel.detailState.characterDescription)
                        .font(.B2)
                        .foregroundStyle(WalkieCommonAsset.gray500.swiftUIColor)
                        .multilineTextAlignment(.center)
                        .padding(.bottom, 20)
                    HStack(spacing: 8) {
                        Text(viewModel.detailState.characterRank.rawValue)
                            .font(.H6)
                            .foregroundStyle(viewModel.detailState.characterRank.fontColor)
                        ...
                        HighlightTextAttribute(
                            text: "\(viewModel.detailState.characterCount)마리 보유",
                            textColor: WalkieCommonAsset.gray500.swiftUIColor,
                            font: .B2,
                            highlightText: "\(viewModel.detailState.characterCount)",
            ...
            CTAButton(
                title: viewModel.detailState.isWalking
                ? "같이 걷는 중..."
                : "이 캐릭터와 같이 걷기",
                style: .primary,
                size: .large,
